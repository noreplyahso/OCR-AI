...
class MainScreen(QMainWindow):
    ...
    def set_event(self):
        ...
        # self.button_connect_PLC.clicked.connect(lambda: signal.connect_PLC.emit(self.line_edit_PLCIP.text(), 1))
        self.button_connect_PLC.clicked.connect(lambda: signal.connect_PLC.emit({
            "protocol_type": "tcp",
        ...
        self.button_save_PLCIP.clicked.connect(self.on_save_PLCIP)
        self.button_reset_counter.clicked.connect(self.on_reset_counter)
        # self.button_reset_counter.clicked.connect(self.on_stop)
        self.button_authentication.clicked.connect(self.open_authentication)
        self.button_move_ROI_left.clicked.connect(lambda: self.on_move_ROI(direction="left"))
        self.button_move_ROI_right.clicked.connect(lambda: self.on_move_ROI(direction="right"))
        self.button_move_ROI_up.clicked.connect(lambda: self.on_move_ROI(direction="up"))
        self.button_move_ROI_down.clicked.connect(lambda: self.on_move_ROI(direction="down"))

    def set_value(self):
        self.live_camera_status = False
        self.real_time_status = False
        self.auto_mode_status = False
        # self.OCR_detect_status = True # Mặc định bật OCR detect 
    ...
    def set_state(self):
        ...
        # QTimer.singleShot(400, lambda: signal.connect_PLC.emit(self.line_edit_PLCIP.text(), 1))
        QTimer.singleShot(400, lambda: signal.connect_PLC.emit({
            "protocol_type": "tcp",
            "ip": self.line_edit_PLCIP.text(),
            "tries": 1
        }))
        ...
        # Đếm số lượng đang hiển thị và kết quả ok/ng hiện tại
            ...
            if result and self.quantity == 5:
                ...
                if not self.check_timer.isActive():
                    # Nếu Checking đang chạy thì ko chạy nữa
    ...
    def right_access(self, main_screen = 1):
            ...
            self.label_current_user.setText(f"User: {current_user[0] if current_user else ''}")
            user = User.get_by("UserName", current_user)
    ...
    def on_grab(self, checked=False):
            ...
            # Nếu ko
                ...
                QTimer.singleShot(result_time*1000, lambda: setattr(self, "live_camera_status", True))
                ...
                if not self.auto_mode_status:
                    self.button_grab.setEnabled(False)
                    QTimer.singleShot(result_time*1000, lambda: self.button_grab.setEnabled(True))
    ...
    def on_goto_training(self, checked=False):
            ...
            exe_path = os.path.join(base_dir, "OCR_DeepLearning_Software", "OCR_DeepLearning_Software.exe")
    ...
    def on_change_product(self, value=None):
            ...
            model_path = df.loc[df["Product name"] == self.current_product, "Model path"].values
            if len(model_path) == 0:
                return
            ...
            if not isinstance(settings, list):
                QMessageBox.warning(self, "Warning", f"Database returned unexpected: {settings}")
                return
            self.spinbox_default_value.setValue(settings[0]["DefaultNumber"]) if settings[0]["DefaultNumber"] else self.spinbox_default_value.setValue(160)
            self.spinbox_exposure_time.setValue(settings[0]["Exposure"]) if settings[0]["Exposure"] else self.spinbox_exposure_time.setValue(3500)
            self.combobox_acceptance_threshold.setCurrentText(str(settings[0]["ThresholdAccept"])) if settings[0]["ThresholdAccept"] else self.combobox_acceptance_threshold.setCurrentText("0.5")
            self.combobox_mns_threshold.setCurrentText(str(settings[0]["ThresholdMns"])) if settings[0]["ThresholdMns"] else self.combobox_mns_threshold.setCurrentText("0.5")
            # print(settings)
            # print(settings[0])
    ...
    @catch_errors  
    def on_save_PLCIP(self, checked=False):
    ...
    def on_move_ROI(self, direction, checked = False):
        # Di chuyển ROI
        if direction == "left":
            if self.checkbox_move_all_ROI.isChecked() or self.spinbox_ROI_to_move.value() == 1:
                self.ROIx1 -= 10
            if self.checkbox_move_all_ROI.isChecked() or self.spinbox_ROI_to_move.value() == 2:
                self.ROIx2 -= 10
            if self.checkbox_move_all_ROI.isChecked() or self.spinbox_ROI_to_move.value() == 3:
                self.ROIx3 -= 10
            if self.checkbox_move_all_ROI.isChecked() or self.spinbox_ROI_to_move.value() == 4:
                self.ROIx4 -= 10
            if self.checkbox_move_all_ROI.isChecked() or self.spinbox_ROI_to_move.value() == 5:
                self.ROIx5 -= 10
